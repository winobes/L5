void complete_cycle(Animatic *ani, int c) {
    ...
    if (!ani[c].flag && (ani[c].frame == 0)) {
        // if flag off and at the end of the loop, turn animation off
        ani[c].is_running = false;
    } else {
        ani[c].is_running = true;
    }
    ani[c].draw = ani[c].is_running;
    // if the animation is running, there is always something to draw ->
    //  there is no blank frame in the loop, so
    return;
}
